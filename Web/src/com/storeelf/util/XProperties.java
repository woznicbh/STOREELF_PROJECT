...
public class XProperties extends Properties {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 7033227699014492519L;
	 ...
	 * are then checked. The method returns <code>null</code> if the property is
	 * not found.
	 * 
	 * @param key
	 *            the property key.
	...
	private String getProperty(String key, int level) {
		...
		if (value != null) {
			...
			while (startName != -1) {
				...
				String constName = value.substring(
						startName + START_CONST.length(), endName);
				...
				if (constValue == null) {
					...
					if (constValue == null) {
						constValue = System.getenv().get(constName);
						if (constValue == null) {
							return value;
						}
					}
